 ...
 : ((appNat X).hom ≫ (α / J).map ((re u).map f) ≫ (appNat Y).inv).1 ≫ Cart u ((α / I).obj Y) =
   Cart u ((α / I).obj X) ≫ ((α / I).map f).1 := by
      ...
      have obs1 : (appNat Y).inv.1 ≫ (Cart u ((α / I).obj Y)) = (α%).map (Cart u Y)  := by
        ...
        let myIsoInv : myiso.inv = (appNat Y).inv.1 := rfl
        have this : myiso.hom = (appNat Y).hom.1 := rfl
        apply (CategoryTheory.Iso.cancel_iso_hom_left (myiso) _ _).1
        rw [← Category.assoc, ← myIsoInv , eq_whisker (Iso.hom_inv_id myiso) _ , Category.id_comp]
        symm
        rw [this, obs3' (Z:=Y)]
